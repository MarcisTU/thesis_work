...
parser.add_argument('-coef_beta', default=15, type=float)
...
class ModelD(torch.nn.Module):
    def __init__(self):
        ...
        self.mlp = torch.nn.Sequential(
            torch.nn.Linear(in_features=64, out_features=3), # 1 = fake_source, 2 = fake_target, 3 = real_target
...
for epoch in range(1, EPOCHS):
    ...
    for x_s, label_s in tqdm(data_loader_source, desc=stage):
        ...
        if n == 0:
            ...
            loss_const = torch.mean((z_s - z_z_s)**2) # can be cosine distance, l1 ..
            loss_tid = torch.mean((x_t - g_t)**2) # reconstruction
