...
class Model(torch.nn.Module):
    ...
    def forward(self, x: PackedSequence, hidden=None):
        ...
        ###
        out, hidden = self.gru_block_1.forward(out_seq, hidden)
...
model = Model()
model.load_state_dict(torch.load(f'./pretrained/model_{RNN_NAME}_zoneout/model-{RNN_NAME}_zoneout.pt', map_location='cpu'))
model = model.train()
torch.set_grad_enabled(False)

text = input("Enter start of a sentence: ")
print("You entered: " + text)
text = text.lower()

sentence = []
for input_word in text.split():
    for index, key in enumerate(dataset_full.vocabulary_keys):
        if input_word == key:
            sentence.append(index)

sentence_length = len(sentence)
x_roll = torch.LongTensor(sentence).unsqueeze(dim=0)
...
for t in range(dataset_full.max_sentence_length):
    ...
    x_packed = pack_padded_sequence(x_roll[:, t].unsqueeze(dim=0), lengths, batch_first=True)

    y_prim, hidden = model.forward(x_packed, hidden)

    if t >= sentence_length-1:  # only start adding new predicted words after sequence is passed through model
        y_prim_unpacked, _ = pad_packed_sequence(y_prim, batch_first=True)
        y_prim_idx = y_prim_unpacked[:, -1:].argmax(dim=-1)  # take only last predicted word
