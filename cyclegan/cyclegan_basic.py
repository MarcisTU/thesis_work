...
parser.add_argument('-learning_rate_d', default=3e-5, type=float)
...
parser.add_argument('-coef_i', default=5, type=float)
...
optimizer_D = torch.optim.Adam(params_D, lr=args.learning_rate_d)
...
for epoch in range(1, EPOCHS):
    ...
    for x_s, label_s in tqdm(data_loader_source, desc=stage):
        ...
        if n % 3 == 0:
            ...
            g_t_prim = model_G_s_t.forward(g_s)
            loss_c_t = torch.mean(torch.abs(g_t_prim - g_t)) * args.coef_t

            loss_c = loss_c_s + loss_c_t

            i_t = model_G_s_t.forward(g_t_prim)
            i_s = model_G_t_s.forward(x_s)
            loss_i_t = torch.mean(torch.abs(i_t - g_t_prim)) * args.coef_i
            ...
            loss_g_t = torch.mean(y_g_t ** 2) # y_g_t -> 0 == Fake
